...
fun EvaluationNavigator(
    ...
    val allQuestions = remember(evaluations) {
        evaluations.flatMap { it.measurement_objects }
    }
    ...
    val currentEval = evaluations.find { it.measurement_objects.contains(currentObject) }
    var title = stringResource(Res.string.evaluation_title)
    currentEval?.measurement_name.let {
        title += " - $it"
    }
